...
function savedefaultbrowser(browser)
  ...
  if MSDOS then
    ...
    if ~or(DefaultBrowser==['Default Windows Browser','Scilab Browser']) then
      DefaultBrowser='Scilab Browser';
    end
  else
    FileBrowser=SCIHOME+"/Browser.ini";
    if ~or(DefaultBrowser==['Scilab Browser','nautilus','mozilla/netscape (gnome-moz-remote)','opera','quanta (kde)']) then
      if with_gtk() then
