...
//-----------------------------------------------------------------------------
  ...
  for i=1:nf 
    ...
    unix_c(make_command+makename + ' '+ files(i)); 
  end
  // then the shared library 
  write(%io(2),'   building shared library (be patient)');
  unix_c(make_command+makename + ' '+ lib_name); 
  ...
  if with_lcc()==%T then
    ...
    select comp_target
     case 'VC++' then 
      ...
      comps=['msvc80express';'msvc80pro';'msvc80std']
      comps=[comps;'msvc90express';'msvc90pro';'msvc90std';'msvc100express'];
      if or(vcvompilerversion==comps)
