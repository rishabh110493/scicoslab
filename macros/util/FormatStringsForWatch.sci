...
// All this is used in Scipad for the watch window of the debugger and
// the tooltips showing user variables content.
// Authors: François Vogel, 2004-2010, Enrico Segre 2007-2008
  ...
  if execstr("tvar = type(var);","errcatch") <> 0 then
    ...
    if and(tvar<>[1 2 4 5 6 7 8 9 10 11 13 14 15 16 17 128 129 130]) then
      ...
      if or(tvar==[1 2 4 5 6 7 8 9 10]) then
        ...
        toolarge=%f
        reasonablesizelimit = 500; // Maximum number of elements in matrices, above which they will
                                   // not be displayed in tooltips nor watched nor editable
                                   // This is to avoid exceeding stacksize when formatting huge
                                   // strings with MatrixToFullPrecisString - performance
                                   // with those huge strings was awful anyway
        if nr*nc > reasonablesizelimit then
          svar = noedit_l + LocalizeForScipad("too large to be displayed") + noedit_r
          editable="false";
          toolarge=%t
        end
      end
...
//      case 0 then
// <TODO> type 0 is not documented yet, see Scilab bug 10307
