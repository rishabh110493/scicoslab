...
function ilib_link_gen_Make_win32(names,files,libs,Makename,libname,ldflags, ...
  ...
  if cc<>"" then 
    ...
    mfprintf(fd,"CFLAGS = $(CC_OPTIONS) -DFORDLL -I\""$(SCIDIR)/routines\"""+...
	   " -Dmexfunction_=mex\%|fF_  -DmexFunction=mex_\%|fF "+ cflags +" \n"); 
    mfprintf(fd,"FFLAGS = $(FC_OPTIONS) -DFORDLL -I\""$(SCIDIR)/routines\"""+...
	   " -Dmexfunction=mex\%|fF "+ fflags +"\n"); 	   
...
// INRIA 2004
  ...
  mfprintf(fd,"\nall :: $(LIBRARY).dll\n");
  mfprintf(fd,"\n$(LIBRARY).dll: $(OBJS)\n");
  mfprintf(fd,ascii(9)+"$(DUMPEXTS) -o ""$(LIBRARY).def"" ""$*"" $(OBJS)\n");
  mfprintf(fd,ascii(9)+"$(LINKER) $(LINKER_FLAGS) $(OBJS) $(OTHERLIBS) $(SCIIMPLIB) $(XLIBSBIN) $(TERMCAPLIB) $(EXTRA_LDFLAGS) $*.def -o $(LIBRARY).dll\n\n");
  ...
  for x=files(:)' ;
  	...
  	mfprintf(fd,"%s$(O):\n",x);
  	if (flag =='c') then
  		mfprintf(fd,ascii(9)+"$(CC) $(CFLAGS) $*.c\n\n");
  	else
  		mfprintf(fd,ascii(9)+"@$(SCIDIR1)\\bin\\f2c.exe $*.f \n");
		mfprintf(fd,ascii(9)+"@$(CC) $(CFLAGS) $*.c \n");
		mfprintf(fd,ascii(9)+"del $*.c \n");
	end
  end
	
  mfprintf(fd,"clean:\n");
  mfprintf(fd,ascii(9)+"del *.obj\n");
  mfprintf(fd,ascii(9)+"del *.dll\n");
  mfprintf(fd,ascii(9)+"del *.lib\n");
  mfprintf(fd,ascii(9)+"del *.def\n");
