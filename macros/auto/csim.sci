...
//!
  ...
  if and(sltyp<>['state-space' 'rational']) then 
    error("csim: Invalid input parameter (3rd)")
  end
  if sltyp=='rational' then sl=tf2ss(sl),end
  if sl.dt<>'c' then 
    warning(msprintf(("%s: Input argument %d is assumed continuous time.\n"),"csim",1));
   end
  //
  [a,b,c,d]=sl(2:5);
  if type(d)==2&degree(d)>0 then 
    d=coeff(d,0);
    warning(msprintf(("%s: Direct feedthrough D set to its zero degree coefficient.\n"),"csim"));
  end
  [maxx,mb]=size(d);
  [ma,mbxx]=size(a);
  //
  ...
  case 10 then //input given by its type (step or impuls)
    ...
    if part(u,1)=='i' then
      ...
      imp=1;
      if norm(d,1)<>0 then
	warning(msprintf(("%s: Direct feedthrough set to zero.\n"),"csim"));
	d=0*d;
      end;
...
//      end;
  ...
  case 1 then //input given by a vector of data
    ...
    if mbu<>mb | ntu<>size(dt,'*') then 
      error(msprintf(("%s: Incompatible input arguments #%d and #%d: Same column dimensions expected.\n"),"csim",1,2))
    end
