...
//!
  ...
  if and(typeof(h)<>['state-space' 'rational'])
   error(msprintf(("%s: Wrong type for input argument #%d: Linear state space or a transfer function expected.\n"),"calfrq",1))
  end
  ...
  if type(dom)==1 then 
    ...
    if fmax>nyq_frq then 
      warning(msprintf(("%s: Frequencies beyond Nyquist frequency are ignored.\n"),"calfrq"));
      fmax=min(fmax,nyq_frq)
    end
    if fmin<-nyq_frq then 
      warning(msprintf(("%s: Negative frequencies below Nyquist frequency are ignored.\n"),"calfrq"));
  ...
  elseif fmin >= fmax then
    error(msprintf(("%s: Wrong value for input arguments #%d and #%d: %s < %s expected.\n"),..
