...
//      message(["%scicos_contrib not defined"; "using default values"])
  ...
  if rhs>=1 then //** scicos_new(...) is called with some arguments
    ...
    if type(scs_m)==10 then //diagram is given by its filename
      ...
      if with_tk() then
        ...
        if with_ttk() then
          //do_pal_tree=do_pal_ttktree
          //do_placeindiagram=do_ttkplaceindiagram
          tk_mdialog=ttk_mdialog
  ...
  if ~super_block then
	...
	deff('x=choose(varargin)', 'x=x_choose(varargin(1:$))');
      ...
      global %cosbrowsehelp //use default help browser for scicos
      global %browsehelp
      %browsehelp_sav=%browsehelp
      if %cosbrowsehelp==[] then
        if ~MSDOS then
          %browsehelp="Scilab Browser"
        end
      else
        %browsehelp=%cosbrowsehelp
      end
      clear %browsehelp
      clear %cosbrowsehelp
