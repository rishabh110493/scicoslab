...
nodeborder=g('node_border');
if nodeborder==[] then nodeborder=g('default_node_border')*ones(1,n);end;
spp=0*ones(n,n);
for i=1:ma
...
edgewidth=g('edge_width');if edgewidth==[] then edgewidth=1*ones(1,ma);end;
...
if (rep(3)==1) then,
  ...
  ii=find(nodetype==1);kk=size(ii);
  for i=1:kk(2)
    iii=ii(i);
    mesu=nodediam(iii);x1=xnodes(iii);y1=-ynodes(iii)+3*mesu;
    xset('pattern',ncolor(iii));
    xrect(x1-0.5*mesu,y1- 4.5*mesu,mesu,mesu);
    ar(1,1)=x1-mesu;ar(2,1)=x1+mesu;ar(3,1)=x1;ar(4,1)=ar(1,1);
    br(1,1)=y1 - 4.5*mesu;br(2,1)=br(1,1);br(3,1)=y1- 3.5*mesu;br(4,1)=br(1, 1);
  ...
  for i=1:kk(2)
    iii=ii(i);
    mesu=nodediam(iii);x1=xnodes(iii);y1=-ynodes(iii);
    xset('pattern',ncolor(iii));
    xrect(x1-0.5*mesu,y1+1.5*mesu,mesu,mesu);
    ar(1,1)=x1-mesu;ar(2,1)=x1+mesu;ar(3,1)=x1;ar(4,1)=ar(1,1);
...
th=xget('thickness');
for i=1:size(metarcs,2) 
  xset('thickness',nodeborder(i));
  xarcs(metarcs(:,i),ncolor(i));
end
xset('thickness',th);
