...
//           'else sin(x)/x,end'],'x',0,%pi)
  ...
  if size(%x0,'*')<>1 then
    error(msprintf(("%s: Wrong size for input argument #%d: A real expected.\n"),"integrate",3));
  end
  ...
  if imag(%x0)<>0 then
    error(msprintf(("%s: Wrong type for input argument #%d: A real expected.\n"),"integrate",3));
  end
  ...
  try
    ...
    error(msprintf(("%s: Wrong value for imput argument #%d: syntax error in given expression\n"),"integrate",1));
  end
