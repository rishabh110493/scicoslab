...
#define stk(x)  (((double *)        (*((double **) &C2F(stack).stkptr))) + (x)-1 )
#define istk(x) (((int *)           (*((double **) &C2F(stack).stkptr))) + (x)-1 )
#define sstk(x) (((float *)         (*((double **) &C2F(stack).stkptr))) + (x)-1 )
#define cstk(x) (((char *)          (*((double **) &C2F(stack).stkptr))) + (x)-1 )
#define zstk(x) (((doublecomplex *) (*((double **) &C2F(stack).stkptr))) + (x)-1 )
#else 
...
#ifndef FTable_H
extern  void * GetFuncPtr (char *,int,void *,void (*f)(),int *,int*,int*);
#endif 
...
#define GetType(n)   C2F(gettype)((c_local = n + Top_stack - Rhs, &c_local))
...
#define OverLoad(n) C2F(overload)((c_local = n + Top_stack - Rhs, &c_local),fname,strlen(fname))
...
#define SetWorkSize(n,mx) if(! C2F(setworksize)((c_local=n,&c_local),mx,1)) \
    { return 0;  }
...
#define Errorinfo(routinename,info)					\
  C2F(errorinfo)(routinename,(c_local=info, &c_local), strlen(routinename));
...
#define CreateCVarFrom(n,ct,it,mx,nx,lx,lc,lx1,lc1)			\
  if (!C2F(createcvarfrom)((c_local=n,&c_local),ct,it,mx,nx,lx,lc,lx1,lc1,1L))  { return 0;} 

#define Createlist(m,n) C2F(createlist)((c_local=m,&c_local),(c1_local=n,&c1_local))

#define CreateListVarFrom(n,m,ct,mx,nx,lx,lx1)				\
  if (!C2F(createlistvarfrom)((c_local=n,&c_local),(c1_local=m,&c1_local),ct,mx,nx,(void *)lx,(void *) lx1,1L)) \
    { return 0;} 

#define CreateListCVarFrom(n,m,ct,it,mx,nx,lx,lc,lx1,lc1)		\
  if (!C2F(createlistcvarfrom)((c_local=n,&c_local),(c1_local=m,&c1_local),ct,it,mx,nx,(void *)lx,(void *) lc,(void *) lx1,(void *)lc1,1L))  { return 0;} 
...
extern int C2F(getmatdims) (integer *, integer *, integer *);
extern int C2F(getrhsvar) (integer *, char *type, integer *, integer *, integer *, unsigned long);
extern int C2F(getrhscvar) (integer *, char *type,integer*,integer*, integer *, integer *, integer *, unsigned long);

extern int C2F(createvar) (integer *, char *, integer *, integer *, integer *, unsigned long );
extern int C2F(createcvar) (integer *, char *,integer *,integer*, integer *, integer *, integer *, unsigned long );

extern int C2F(putlhsvar) (void);
extern int C2F(cmatptr) (char *,integer *,integer *,integer *,unsigned long);
extern int C2F(createcvarfromptr) (integer *,char *,integer*, integer *, integer *,double *,double *, unsigned long);

extern int C2F(scifunction) (integer *,integer *,integer *,integer*);

extern int C2F(firstopt) (void);
extern int C2F(findopt) (char *, rhs_opts *);

extern int C2F(numopt) (void);
extern int C2F(isopt) (integer *,char *,unsigned long);
extern int C2F(gettype) (integer *pos);
...
extern void *GetData (int lw);

extern void *GetRawData (int lw);

extern void *GetDataFromName ( char *name);

extern int GetDataSize (int lw);

/* to hide warnings */
static void _hide_unused_c_local(int x);
static void _hide_unused_c1_local(int x)
{
  x = c1_local; _hide_unused_c_local(x);
}
static void _hide_unused_c_local(int x) 
{
  x = c_local; _hide_unused_c1_local(x);
}
