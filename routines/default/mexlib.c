...
static int the_lock_status=0;

extern void cerro (const char *str);
extern int C2F(dcopy) (int*, double *, int *, double *, int *);
extern int C2F(xscion)  (int *i);
extern void C2F (xscisncr) (char *str, integer *n, integer dummy); 
extern int  C2F(mxgetm) (    mxArray *ptr);
extern int  C2F(mxgetn) (    mxArray *ptr);
extern void C2F(xscisrn) (char *str,int *n,int  dummy);
extern int  C2F(erro)  (char *str,int dummy);
...
/*    set ARRAYPERSIST to 1 to unable mexMakeArrayPersistent */
#define ARRAYPERSIST 0
...
static int *Header(const mxArray *ptr)
{
  ...
  for ( i = 0 ; i < rec_size ; i++)
    {
      if ((calloc_table[i].adr == ptr) && (calloc_table[i].keep == 2))
	return (int *) ptr;
    }
#endif
...
#if 0
static mxArray *header2ptr(int *header)
{
  mxArray ptr;
...
/* used in scicos: this function should not be there in mexlib.c 
 *
 */
...
static int theMLIST(int *header)
{
...
int mxGetNumberOfElements(const mxArray *ptr)
{
  ...
  int *header=Header(ptr);
  switch (header[0]) {
  case DOUBLEMATRIX: case INTMATRIX: case LOGICAL:
...
int mxGetNumberOfDimensions(const mxArray *ptr)
{
  ...
  int *header = Header(ptr);
  switch (header[0]) {
  case DOUBLEMATRIX: case INTMATRIX: case SPARSEMATRIX: case STRINGMATRIX: case LOGICAL:
    return 2;
  case MLIST:
    switch (theMLIST(header)) {
    case NDARRAY:
...
int *mxGetDimensions(const mxArray *ptr)
{
  ...
  int *header = Header(ptr);
  switch (header[0]) {
  case DOUBLEMATRIX: case INTMATRIX: case STRINGMATRIX: case LOGICAL:
    return &header[1];
  case SPARSEMATRIX: /* to be done  */
    return &header[1];
    break;
...
int mxGetM(const mxArray *ptr)
{
  ...
  int *header = Header(ptr);
  switch (header[0]) {
  case DOUBLEMATRIX: case INTMATRIX: case STRINGMATRIX: case SPARSEMATRIX: case LOGICAL:
    return header[1];
  case MLIST:
    switch (theMLIST(header)) {
    case NDARRAY:
...
int mxGetN(const mxArray *ptr)
{
  ...
  int *header = Header(ptr);
  switch (header[0]) {
  case DOUBLEMATRIX: case INTMATRIX: case SPARSEMATRIX: case LOGICAL:
    return header[2];
  case STRINGMATRIX:
...
void mxAssert(int expr, const char *error_message)
{
...
{
  int lw, lr, lc, k,it1 = it;
  lw = Nbvars + 1;
  if (! C2F(createcvar)(&lw, "d",&it1, &m, &n, &lr, &lc, 1L)) {
    mexErrMsgTxt("No more memory available: increase stacksize");
  }
  for ( k=0; k<m*n*(it+1); k++ ) {
   *stk(lr+k)=0;
  }
  return (mxArray *) C2F(vstk).lstk[lw + Top_stack - Rhs - 1];  /* C2F(intersci).iwhere[lw-1]);  */
}
...
mxArray *mxCreateStructArray(int ndim, const int *dims, int nfields, const char **field_names)
{
  ...
  C2F(intersci).ntypes[lw-1]=AsIs;
  return (mxArray *) C2F(vstk).lstk[lw + Top_stack - Rhs - 1 ];
}
...
void mxSetFieldByNumber(mxArray *array_ptr, int index, int field_number, mxArray *value)
{
  ...
  if (IsReference(value))
  ...
  else 
    {
      ...
      point_ed = Top_stack - Rhs + pointed;
...
{
  int lw,lw1, retval, iclass= CLASS, icmplx=cmplx;
  if (ndim < 3) return mxCreateNumericMatrix(dims[0],dims[1], CLASS, cmplx);
  Nbvars++;
  lw = Nbvars;
  lw1 = lw + Top_stack - Rhs;
  C2F(hmcreate)(&lw1, &ndim, (int *) dims,&iclass,&icmplx,&retval);
  if( !retval) {
    return (mxArray *) 0;
  }
  C2F(intersci).ntypes[lw-1]=AsIs;
...
mxArray *mxCreateNumericMatrix(int m, int n, mxClassID CLASS, int cmplx)
{
  ...
  case mxINT8_CLASS:
    Nbvars++;lw = Nbvars;  lw1 = lw + Top_stack - Rhs;
...
mxArray *mxCreateCellArray(int ndim, const int *dims)
{
  ...
  header =  (int *) stk(C2F(vstk).lstk[lw + Top_stack - Rhs - 1 ]);
...
mxArray *mxGetCell(const mxArray *ptr, int index)
{
  ...
  C2F(intersci).ntypes[lw-1]=AsIs;
  C2F(intersci).iwhere[lw-1]=C2F(vstk).lstk[lw+ Top_stack - Rhs - 1];
  return (mxArray *) C2F(intersci).iwhere[lw-1];
}
...
void mxFreeMatrix(mxArray *ptr)
{
  ...
  if ( (int)ptr == C2F(vstk).lstk[Top_stack - Rhs + Nbvars - 1]) {
      ...
      * and its size in Scilab double stack */
{
  ...
  for (kk = 1; kk <= Nbvars; kk++)
    {
      *number=kk;
      if (lst_k == C2F(vstk).lstk[kk+ Top_stack - Rhs -1]) break;
    }
  *size = C2F(vstk).lstk[*number+ Top_stack - Rhs] - lst_k;
  } else
    {
    *number=0;
...
bool mexIsGlobal(const mxArray *ptr)
{
  int pointed;
  int ret_val;
  int *header= (int *) Header(ptr);
  /* int *rheader = (int *) RawHeader(ptr); */
...
{
  int lw, iprov, icmplx=cmplx;
  Nbvars++;
  lw=Nbvars;
  iprov = lw + Top_stack - Rhs;
  if( ! C2F(mspcreate)(&iprov, &m, &n, &nzmax, &icmplx)) {
    return (mxArray *) 0;
  }
  C2F(intersci).ntypes[lw-1]=AsIs;
  return (mxArray *) C2F(vstk).lstk[lw + Top_stack - Rhs - 1];     /* C2F(intersci).iwhere[lw-1])  */
...
mxArray *mxCreateString(const char *string)
{
  ...
  if ( ! C2F(createvarfromptr)(&lw, "c", &one, &i, &string, 1L) ) {
    return (mxArray *) 0;
  }
...
static int mexCallSCI(int nlhs, mxArray **plhs, int nrhs, mxArray **prhs, char *name,int jumpflag)
{
  ...
  for (k = 1; k <= nrhs; ++k) 
    {
      for (kk = 1; kk <= nv; ++kk) 
	if ((int) prhs[k-1] == C2F(vstk).lstk[kk + Top_stack - Rhs - 1]) break;
      if (kk == nv + 1) 
	{
  ...
  for (k = 1; k <= nlhs; ++k) {
    plhs[k-1] = (mxArray *) C2F(vstk).lstk[nv + k + Top_stack - Rhs - 1];
  }
  Nbvars = nv+nlhs;
  return 0;
}
...
#ifdef ARRAYPERSIST
mxArray *mexMakeArrayPersist(mxArray *ptr)
{
  int number, size, k;
  double *old , *data;
  numberandsize(ptr, &number, &size);
  old = stk( (int) ptr );
  ptr = mxMalloc_m( size*sizeof(double) );
...
{
  static int i, lr1, lw,lw1, ilocal, n, k,kk, nmax;
  nmax = strlen(str[0]);
  for (k=1; k<m; ++k) {
    n=strlen(str[k]);
    if (n > nmax) nmax=n;
  ...
  C2F(intersci).ntypes[lw1 -1]=AsIs;
...
int C2F(createptr)(char *type, int *m, int *n, int *it, int *lr, int *ptr, long int type_len)
{
  ...
  *ptr = C2F(vstk).lstk[lw + Top_stack - Rhs - 1];
  return 1;
} 
  ...
  for (k = 1; k <= *nlhs; k++)
    {
      ...
      for (kk = 1; kk <= nv; kk++)
	{
	  if (plhsk == C2F(vstk).lstk[kk+ Top_stack - Rhs -1]) 
	    {
	      LhsVar(k) = kk;
	      break;
	    }
...
void clear_mex(integer nlhs, mxArray **plhs, integer nrhs, mxArray **prhs)
{
  ...
  for (k = 1; k <= nv; k++)
    if ( max <  C2F(vstk).lstk[k + Top_stack - Rhs -1]) Nbvars--;
}
...
mxArray * C2F(mxcreatestring)(char *string, long int l)
{
  ...
  return (mxArray *) C2F(vstk).lstk[Top_stack -Rhs + lw - 1];
}
...
/* 
 * Next function are added in order to access 
 * scicos structures. Thus tehy are not 
 * stricly speaking part of mexlib. 
 * 
 * Fady NASSIF: November 2009 
 */
#if 0
int *mxGetHeader (const mxArray *ptr)
{
  /*This function get the header of the scilab object*/
  return Header(ptr);
}

mxArray *mxHeader2Ptr(int *header)
{
  mxArray *ptr;
  ptr=header2ptr(header);
  return ptr;
}
#endif 

static int C2F(createpointer)(int *lw, int *lr,int *  it,int * m,int * n)
{
  integer ix1,lw1,stlw;
  integer il;
  if (*lw > intersiz) {
    Scierror(999,"(crepointerit) too many arguments in the stack edit stack.h and enlarge intersiz\r\n");
    return 0 ;
  }
  Nbvars = Max(*lw,Nbvars);
  if (*lw < 0) {
    Scierror(999,"bad call to crepointerit! (1rst argument)\r\n");
    return 0 ;
  }
  lw1 = *lw + Top_stack - Rhs;
  stlw=*Lstk(lw1);
  ...
  if (Err > 0) {
    Scierror(17,"Creating Pointer: stack size exceeded (Use stacksize function to increase it)\r\n");
    return 0;
  };
  *istk(il ) = 128;
  *istk(il + 1) = *m;
...
mxArray *mxCreatePointer(int m,int n,double *p,mxPointerID it)
{
  int new,lr=0;
  int it1=it;
  Nbvars++;
  new=Nbvars;
  if (!C2F(createpointer)(&new, &lr, &it1, &m, &n))
    mexErrMsgTxt("No more memory available: increase stacksize");
  *stk(lr)=*p;
  return (mxArray *) C2F(intersci).iwhere[new-1];
}

mxPointer mxGetPointerPr(const mxArray *ptr)
{
  int *header = Header(ptr);
  if (header[0]==POINTER)
    return (mxPointer) header[4];
  else return NULL;
}

void mxSetPointerPrToNull(mxArray *ptr)
{
  int *header = Header(ptr);
  if (header[0]==POINTER)
    {
      header[3]=mxNULLPOINTER;
      header[4]=0;
    }
  else
    {
      mexErrMsgTxt("(mxSetPointerPrToNull) :input is not a pointer");
    }
}

mxPointerID mxGetPointerID(const mxArray *ptr)
{
  int *header = Header(ptr);
  if (header[0] != POINTER)
    mexErrMsgTxt("(mxGetPointerId) :input is not a pointer");
  return header[3];
}

bool mxIsPointer(const mxArray *ptr)
{
  int *header = Header(ptr);
  return header[0] == POINTER;
}

bool mxIsDiagram(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxDIAGRAMPOINTER;
}

bool mxIsBlock(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxBLOCKPOINTER;
}

bool mxIsSubSystem(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxSBLOCKPOINTER;
}

bool mxIsLink(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxLINKPOINTER;
}

bool mxIsCpr(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxCPRPOINTER;
}

bool mxIsXord(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxXORDPOINTER;
}

bool mxIsCorinv(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxCORPOINTER;
}
bool mxIsNULL(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxNULLPOINTER;
}

bool mxIsCprDiagram(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxCPRDIAGPOINTER;
}

bool mxIsSimParams(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxSIMPARAMSPOINTER;
}

bool mxIsModel(const mxArray *ptr)
{
  return mxGetPointerID(ptr)==mxMODELPOINTER;
}
