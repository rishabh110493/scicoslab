...
extern void * MyStackAlloc(unsigned int dwSize,char *fichier,int ligne);
extern void MyStackFree(void *x, char *fichier,int ligne);
   ...
   #define SCISTACKMALLOC(x) MyStackAlloc((size_t)x,__FILE__,__LINE__)
   #define SCISTACKFREE(x) if (x  != NULL) MyStackFree((void *) x,__FILE__,__LINE__);
   ...
   #define CALLOC(x,y) MyCalloc((unsigned) x, (unsigned) y,__FILE__,__LINE__)
    ...
    /* Only for Scilab Stacksize use VirtualAlloc on Window */
   #define SCISTACKMALLOC(x) MyStackAlloc((unsigned)x,__FILE__,__LINE__)
   #define SCISTACKFREE(x) if (x  != NULL) MyStackFree((char *) x,__FILE__,__LINE__);
